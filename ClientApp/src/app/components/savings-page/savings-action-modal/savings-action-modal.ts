...
import { SavingsAllocation, SavingsCategory, SavingsCategoryId, SavingsTransaction } from '../../../models/savings.model';

export type SavingsModalMode = 'bulk' | 'transfer' | 'top-up' | 'spend' | 'allocation' | 'edit';
...
export class SavingsActionModalComponent implements OnChanges {
    ...
    @Input() errorMessage = '';
    @Input() transactionToEdit: SavingsTransaction | null = null;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (changes['transactionToEdit'] && this.transactionToEdit) {
            ...
            this.fromCategoryIdValue = this.transactionToEdit.fromCategoryId ?? '';
            this.toCategoryIdValue = this.transactionToEdit.toCategoryId ?? '';
        } else if (changes['transactionToEdit']) {
            this.amountValue = null;
            this.noteValue = '';
            this.fromCategoryIdValue = '';
            this.toCategoryIdValue = '';
        }
    }

    protected modalTitle(): string { return ({ bulk: 'Add to all envelopes', transfer: 'Move funds', 'top-up': 'Top up envelope', spend: 'Spend from envelope', allocation: 'Edit allocation', edit: 'Edit transaction' } as Record<string, string>)[this.mode ?? ''] ?? ''; }
    protected modalActionLabel(): string { return this.mode === 'transfer' ? 'Transfer funds' : this.mode === 'spend' ? 'Record spending' : this.mode === 'allocation' ? 'Save allocation' : this.mode === 'edit' ? 'Save changes' : 'Save transaction'; }
